...
public class ChatRoomReponsitoryImpl implements ChatRoomReponsitory {
    ...
    public List<Member> findUser(Long id) throws Exception {
        String sql = " Select mem.id, mem.username, mem.status, mem.description from member mem where mem.status= :status and mem.id <> :id";
        RawSql rawSql = RawSqlBuilder.parse(sql).create();

        List<Member> list = Ebean.find(Member.class).setRawSql(rawSql)
            .setParameter(CommonConsts.STATUS, CommonConsts.VALUE_STATUS_ONE)
            .setParameter(CommonConsts.ID, id).findList();
        return list;
    }
    ...
    public List<Groups> findAllGroup(Long id) throws Exception {
        ...
        sql.append(
            " Select grp.groupsname, grp.id, grp.status, grp.description from groups grp join membersgroup mgr on mgr.id = grp.id where grp.status= :status and mgr.memberid = :id and mgr.status = :status ");
        RawSql rawSql = RawSqlBuilder.parse(sql.toString()).create();

        List<Groups> listGroup = Ebean.find(Groups.class).setRawSql(rawSql)
            .setParameter(CommonConsts.STATUS, CommonConsts.VALUE_STATUS_ONE)
            .setParameter(CommonConsts.ID, id)
            .setParameter(CommonConsts.STATUS, CommonConsts.VALUE_STATUS_ONE).findList();
        return listGroup;
    }
    ...
    public List<Groups> findAllUserGroup(Long id) throws Exception {
        StringBuilder sql = new StringBuilder();
        sql.append(
            "Select grp.groupsname, grp.id, grp.status, grp.description from groups grp, membersgroup mgr where grp.id = mgr.groupid and mgr.memberid = :id and mgr.status = :status");
        RawSql rawSql = RawSqlBuilder.parse(sql.toString()).create();
    ...
    public List<Member> selectMemberGroup(Long id) throws Exception {
        String sql = " Select mem.username from member mem join membersgroup memgroup on mem.id = memgroup.memberid where memgroup.status= :status and memgroup.groupid = :groupid";
        RawSql rawSql = RawSqlBuilder.parse(sql).create();
    ...
    public MembersGroup getMembersGroupById(Long groupId, Long memberId) throws Exception {
        StringBuilder sql = new StringBuilder();
        sql.append(
            "Select mgr.id, mgr.groupId, mgr.memberId, mgr.status from MembersGroup mgr where mgr.groupid = :groupId and mgr.memberId = :memberId and status = :status");
        RawSql rawSql = RawSqlBuilder.parse(sql.toString()).create();

        MembersGroup membersGroup = Ebean.find(MembersGroup.class).setRawSql(rawSql)
            .setParameter("groupId", groupId).setParameter("memberId", memberId)
            .setParameter("status", CommonConsts.VALUE_STATUS_ONE).findOne();
        return membersGroup;
    }
    ...
    public Groups getGroupById(Long groupId) throws Exception {
        StringBuilder sql = new StringBuilder();
        sql.append(
            "select g.id, g.groupname, g.status, g.description from groups g where g.id = :groupId and status = :status ");
        RawSql rawSql = RawSqlBuilder.parse(sql.toString()).create();

        Groups Group = Ebean.find(Groups.class).setRawSql(rawSql).setParameter("groupId", groupId)
            .setParameter("status", CommonConsts.VALUE_STATUS_ONE).findOne();
        return Group;
    }
    ...
    public List<MembersGroup> getListMembersGroup(Long groupId) throws Exception {
        StringBuilder sql = new StringBuilder();
        sql.append(
            "Select mgr.id, mgr.groupId, mgr.memberId, mgr.status from MembersGroup mgr where  mgr.groupId = :groupId and status = :status");
        RawSql rawSql = RawSqlBuilder.parse(sql.toString()).create();

        List<MembersGroup> lstMembersGroup = Ebean.find(MembersGroup.class).setRawSql(rawSql)
            .setParameter("groupId", groupId).setParameter("status", CommonConsts.VALUE_STATUS_ONE)
            .findList();
